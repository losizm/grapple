...
import scala.jdk.CollectionConverters.{ IteratorHasAsJava, ListHasAsScala, SeqHasAsJava }
...
private class CombinedJsonArray(left: JsonArray, right: JsonArray) extends JsonArray {
  ...
  def iterator(): jutil.Iterator[JsonValue] = arr.iterator.asJava
  def listIterator(): jutil.ListIterator[JsonValue] = arr.asJava.listIterator
  def listIterator(index: Int): jutil.ListIterator[JsonValue] = arr.asJava.listIterator(index)
  def subList(fromIndex: Int, toIndex: Int): jutil.List[JsonValue] = arr.slice(fromIndex, toIndex).asJava
  def toArray(): Array[AnyRef] = arr.toArray[AnyRef]
  def toArray[T](buf: Array[T with AnyRef]): Array[T with AnyRef] = arr.asJava.toArray[T](buf)
