...
object Implicits {
  ...
  implicit def collectionJsonInput[A, M[A]](implicit input: JsonInput[A], factory: Factory[A, M[A]]) =
    new JsonInput[M[A]] {
      def reading(json: JsonValue): M[A] =
        json.asInstanceOf[JsonArray].asScala.map(input.reading).to(factory)
    }
  ...
  /** Creates JsonOutput for converting IterableOnce to JsonArray. */
  implicit def iterableOnceJsonOutput[A, M[A] <: IterableOnce[A]](implicit output: JsonOutput[A]) =
    new JsonOutput[M[A]] {
  ...
  implicit class JsonValueType(private val json: JsonValue) extends AnyVal {
    ...
    def \\(name: String): Seq[JsonValue] =
      json match {
        ...
        case obj: JsonObject => Option(obj.get(name)).toSeq ++: obj.values.asScala.flatMap(_ \\ name).toSeq
        case _               => Nil
      }
