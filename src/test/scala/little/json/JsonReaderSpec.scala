...
class JsonReaderSpec extends org.scalatest.flatspec.AnyFlatSpec:
  private val textObject = """{ "id": 1000, "name": "jza", "groups": ["jza", "adm", "sudo"] }"""
  private val textArray  = """[1000, "jza", ["jza", "adm", "sudo"]]"""
  ...
  it should "parse JSON text" in {
    ...
    assert((obj \ "name")   == JsonString("jza"))
    assert((obj \ "groups") == JsonArray(Seq("jza", "adm", "sudo")))

    val arr = Json.parse(textArray).as[JsonArray]
    assert(arr.size == 3)
    assert((arr \ 0) == JsonNumber(1000))
    assert((arr \ 1) == JsonString("jza"))
    assert((arr \ 2) == JsonArray(Seq("jza", "adm", "sudo")))
  ...
  it should "parse JSON file" in {
    ...
    assert(json1.names == Seq("root", "guest", "jza"))
    ...
    assert((json1 \ "jza" \ "id")     == JsonNumber(1000))
    assert((json1 \ "jza" \ "name")   == JsonString("jza"))
    assert((json1 \ "jza" \ "groups") == JsonArray(Seq("jza", "adm", "sudo")))

    val json2 = Json.parse(Paths.get("./src/test/resources/users.json")).as[JsonObject]
    assert(json2.names == Seq("root", "guest", "jza"))
    ...
    assert((json2 \ "jza" \ "id")     == JsonNumber(1000))
    assert((json2 \ "jza" \ "name")   == JsonString("jza"))
    assert((json2 \ "jza" \ "groups") == JsonArray(Seq("jza", "adm", "sudo")))
  }
