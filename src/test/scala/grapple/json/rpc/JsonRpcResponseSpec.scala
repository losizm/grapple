...
class JsonRpcResponseSpec extends org.scalatest.flatspec.AnyFlatSpec:
  ...
  it should "create JsonRpcResponse with attributes" in {
    ...
    assert(res1.id.number == 123)
    assert(res1.result.as[Int] == 6)
    assert(res1.attributes.size == 2)
    assert(res1.getOption("one").contains(1))
    assert(res1.getOption("two").contains("2"))
    assert(res1.getOrElse("one", 0) == 1)
    assert(res1.getOrElse("two", "0") == "2")
    assert(res1.getOrElse("three", 0) == 0)
    assert(res1.get[Int]("one") == 1)
    assert(res1.get[String]("two") == "2")
    assertThrows[NoSuchElementException](res1.get[Int]("three"))

    val res2 = res1.put("one", "x").put("three", "xxx")

    assert(res2.id.number == 123)
    assert(res1.result.as[Int] == 6)
    assert(res2.attributes.size == 3)
    assert(res2.getOption("one").contains("x"))
    assert(res2.getOption("two").contains("2"))
    assert(res2.getOption("three").contains("xxx"))

    val res3 = res2.remove("two")
    assert(res3.id.number == 123)
    assert(res1.result.as[Int] == 6)
    assert(res3.attributes.size == 2)
    assert(res3.getOption("one").contains("x"))
    assert(res3.getOption("three").contains("xxx"))

    val res4 = res3.setAttributes(Map("1" -> 1, "2" -> 2))
    assert(res4.id.number == 123)
    assert(res1.result.as[Int] == 6)
    assert(res4.attributes.size == 2)
    assert(res4.getOption("1").contains(1))
    assert(res4.getOption("2").contains(2))
  }
